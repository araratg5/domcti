...
if ($_GET["shop_id"] != "") {
	...
	$di = 0;
	$sql ="SELECT * FROM `import_data_{$shopData['shop_id']}` LIMIT {$_GET['start']},1000";
	$importMasterCount = dbCount($sql);
	if($importMasterCount > 0){
		...
		foreach ($importMasterData as $array) {
			...
			if($head != '電話番号１'){
				...
				if(!$exist[$di]){//重複なし
					$sql = "INSERT INTO `customer_data` (`shop_id`,`usage_shopname`,`modified`,`created`) VALUES ('{$shopData['shop_id']}','{$shopData['name_ja']}','{$insertData[$di]['modified']}','{$insertData[$di]['created']}')";
					...
					$sql ="UPDATE `customer_data` SET {$queryStr} WHERE `id` = '{$insertId}'";
					dbQuery($sql);
					...
					if(strpos($exist[$di]['remark'],$shopData['name_ja']) === false && $itemData[$di][23]!=''){
						$sql ="UPDATE `customer_data` SET `remark` = '{$exist[$di]['remark']}{$insertData[$di]['remark']}' WHERE `id` = '{$exist[$di]['id']}'";
						dbQuery($sql);
					}
				}
				$di++;
		...
		$next = $_GET['start'] + 1000;
		//header("Location:". "/convert.php?shop_id={$_GET['shop_id']}&start={$next}");
	} else {
